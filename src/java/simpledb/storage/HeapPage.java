...
public class HeapPage implements Page {
    ...
    private int getNumTuples() {
        // some code goes here
        double n = java.lang.Math.floor((BufferPool.getPageSize() * 8 * 1.0) / (td.getSize() * 8 + 1));
        return (int) n;
    }
    ...
    private int getHeaderSize() {
        // some code goes here
        double headerBytes = java.lang.Math.ceil(getNumTuples()*1.0 / 8);
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (tuples[tpnum] == null || !isSlotUsed(tpnum)){
            throw new DbException("tuple slot is already empty");
        }
        markSlotUsed(tpnum, false);
        tuples[tpnum] = null;
    }
    ...
    public boolean isSlotUsed(int i) {
        ...
        int indexOfBytes = i/8;
        int indexInByte = i%8;
        byte b = header[indexOfBytes];
        int r = (b>>indexInByte)&0x01;
        if (r == 1){
            return true;
        }else{
            return false;
    ...
    private void markSlotUsed(int i, boolean value) {
        ...
        int indexOfBytes = i/8;
        int indexInByte = i%8;
        byte mask = (byte)(1<<indexInByte);
        if(value){
            header[indexOfBytes] |=  mask;
        }else{
            header[indexOfBytes] &=  ~mask;
        }
    }
